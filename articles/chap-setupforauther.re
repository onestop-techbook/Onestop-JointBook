...
エディタには、Visual Studio CodeにReviewプラグインを入れることをお勧めします。

なお、少し不便ですが、CIがあるのでローカル環境は構築しないという方針もありです。実際筆者は、初めてReviewで本を書いたとき(技術同人誌を書こう-アウトプットのススメの同人版。220ページのうち約40ページ)、エディタ(TeraPad)だけで書きました。ある程度原稿を書いたら、Pushして、CIでコンパイルして確かめる、という書き方です。少々面倒ですが、何とかなります。

== MacでReview
Macについては、著者がMacユーザーではないので割愛します。Dockerをそのまま使えばよいらしいです。すみません…
改版するときには追加します。(あるいは、ぐぐって。)

== SourceTreeを使って原稿を管理する
...
ここでも、コマンドラインでの操作については、私が書けないため割愛します。改版までには自分でもできるように、執筆駆動で習得するようにします…