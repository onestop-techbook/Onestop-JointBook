...
直接Push、PR-セルフマージのメリットを次に列挙します。

また、編集長の立場であれば、プルリクが上がったら、コンフリクトだけチェックしてサクッとマージしてしまいます。ノールックマージ。Githubのプルリク画面で、コンフリクトの有無、CIエラーの有無が確認できますから、両方エラーがなければサクッとマージしてしまいましょう。
...
原稿ファイルであれば、ソフトウエアと異なり「予期せぬ挙動・バグ」が混入するという可能性は殆どありません。CIを走らせていれば、Re:view構文のミスなどはCIで見つけてもらえます（CIでのコンパイル中にエラーが出る、表示がずれる、章のネストが変（これは本人のミスです）等は本人のミスまたは仕様です。これらのエラーは、原則Pushした著者が直すように、と明記・周知すると良いでしょう。Re:viewは（あるいはTeXは）エラーがとにかく分かりづらいので、それらと戦う必要は出ますが、できるだけ著者が直し、編集長がヘルプするという体制を整えておけると、途中の進捗管理は相当楽になるでしょう。
...
コンフリクトも直すのは著者か、編集長になろうかと思いますが、大抵の場合は「両方を生かす」という編集をすれば良いことになります。
...
これは、コンパイルエラーの原因追及などにおいて、わかりづらくなる。の一点につきます。

慣れていない人が試行錯誤して樹形図が複雑になると本来の原因がわかりづらくなります。CIでチェックしていたたとしても、そのエラーメッセージはかなりわかりづらいです。画像や原稿ファイルが見つからないならそう言ってほしいし、吐き出されるログのほとんどはこんなパッケージを読み込みましたのメッセージだったりします。
コミットはこまめにしましょう。コミット回数が増えることを嫌がってはいけません。小刻みなコミットを重ねることで、全体に進捗が見えやすくなります。
...
執筆を進めるうえでの小ネタ的なネタを記載しました。ネタ出しの鉄板メソッド、肉肉野菜肉野菜ご飯、そしてセルフマージを推奨するのがよい。お役に立ちましたら幸いです。執筆を進めるうえでの小ネタ的なネタを記載しました。ネタ出しの鉄板メソッド、肉肉野菜肉野菜ご飯、そしてセルフマージを推奨するのがよい。お役に立ちましたら幸いです。合同誌を回すにあたっての小ネタ集として、ネタ出しの方法と、マスターへのPush許可・セルフマージについて記載しました。全員が楽になる方法ですから、やってみてはいかがでしょう？