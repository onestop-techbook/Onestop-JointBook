...
まず考える必要があるのは、公開リポジトリでいくのか、プライベートリポジトリでいくのかです。公開リポジトリで行う場合、当然ですが原稿ファイルが全世界に公開されます。見る人が見れば、ダウンロードもできますし、pdfをローカルで作ることも可能です。したがって、「勝手にダウンロードされたくない！」という場合はプライベートリポジトリにしましょう。ただし、GitHubの場合はプライベートリポジトリを作れるOrganizationを作るのは有料になります。月9ドル×人数になる斧で、(本稿執筆時点。約1000円/月)ので、ちょっと厳しいかもしれませんね。フリーランスや本業で課金している場合、あるいはほかのGitサービス(GitLab等)を使う場合はPrivateで、そこに作って関係者をし呼ぶという形もいいかもしれません。
...
本書においては、Githubの無料かつPublicリポジトリを使う前提で話を進めます。
...
PDFは参加者全員が同じ環境で作ったpdfが確認できる方が良いですよね？Windowsで書いている人、Mac上で書いている人、WordやMarkdownなどで書いている人、いろいろいますが、今回は最終的にRe:viewにコンバートすることを一旦の前提とします。ローカル環境を作っている人ばかりではありませんし、ローカル環境はそれぞれの人で異なります。Dockerで、なんていう話もありますが、新しく環境構築するのはたいへんです。
...
今回のプロジェクトは合同誌ですから、ほかの執筆者を集める必要があります。Twitterで呼びかける、勉強会などで声をかける、既知のツテを頼って原稿依頼を行う、などで執筆者を集めます。著者にも都合・予定がありますから、できるだけ早く募集を開始し、早めに著者を集めましょう。締め切りはすぐにやってきます。
